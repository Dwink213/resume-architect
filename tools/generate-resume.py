...
def build_resume(
    ...
    cloud_provider_exp = next(e for e in source["experience"] if e["company"] == "National Cloud Provider")
    public_safety_exp = next(e for e in source["experience"] if e["company"] == "Raleigh / County 911")

    align_bullets = select_experience_bullets(align_exp["bullets"], role_tags, jd_terms, max_bullets=4)
    cloud_provider_bullets = select_experience_bullets(cloud_provider_exp["bullets"], role_tags, jd_terms, max_bullets=4)

    # Competency blocks
    comp_blocks = select_competency_blocks(source["competency_blocks"], role_tags, jd_terms, max_blocks=3)

    # Founder work, always include AWACS; include Federal Proposal Consultancy if govcon signal
    founder_awacs = next(f for f in source["founder_work"] if f["id"] == "awacs_llc")
    founder_blue = next(f for f in source["founder_work"] if f["id"] == "federal_proposal")
    include_federal_proposal = "govcon" in role_type or any("govcon" in t for t in role_tags)

    # Open source
    prs = source["open_source"]
    ...
    # --- Assemble ---
    lines = [
        f"# Dustin Winkler, {role_title_hint}",
        ...
        *projects_lines,
        "## EXPERIENCE",
        "",
        # Founder work folded into EXPERIENCE under the standard header so Workday/ATS
        # counts the years and reads AWACS as the *current* role (the consulting reframe).
        # AWACS (2024–Present) leads; Align now ends May 2026.
        f"**{founder_awacs['name']}** | {founder_awacs['location']} | {founder_awacs['dates']}",
        f"*{founder_awacs['role']}*",
        "",
        *[f"- {b}" for b in founder_awacs["bullets"]],
        "",
    ]
    if include_federal_proposal:
        lines += [
            f"**{founder_blue['name']}** | {founder_blue['role']} | {founder_blue['dates']}",
            founder_blue["text"].strip(),
            "",
        ]
    lines += [
        f"**{align_exp['company']}** | {align_exp['location']} | {align_exp['dates']}",
        f"*{align_exp['title']}*",
        "",
    ]
    for b in align_bullets:
        lines.append(f"- {b['text']}")
    lines += [
        "",
        f"**{cloud_provider_exp['company']}** | {cloud_provider_exp['location']} | {cloud_provider_exp['dates']}",
        f"*{cloud_provider_exp['title']}*",
        "",
    ]
    for b in cloud_provider_bullets:
        lines.append(f"- {b['text']}")
    lines += [
        "",
        f"**{public_safety_exp['company']}** | {public_safety_exp['location']} | {public_safety_exp['dates']} | "
        f"*{public_safety_exp['title']}*",
        public_safety_exp["text"].strip(),
        "",
        "---",
        "",
        "## UPSTREAM · ANTHROPICS/CLAUDE-CODE",
        "",
        "My agentic system studied the Claude Code release notes, surfaced these issues, validated "
        "them under my direction, and filed them under my account:",
        "",
    ]
    for pr in prs:
